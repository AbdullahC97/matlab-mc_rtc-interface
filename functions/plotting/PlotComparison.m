...
if plotPos
    for r = 1:length(robotNames)
        ...
        formatDefault.lineWidth = linspace(maxLineWidth,minLineWidth,nSims);
            ...
            for j = 1:numPlotsY
                ...
                if options1.showControlBar &&  j == numPlotsY
                    ...
                    for k = 1:length(DataBase)
                        ...
                        pos(k) = plot(curAxis,DataBase(k).Time,curRobot.realPos.(fitDir(j)),'LineWidth',formatDefault.lineWidth(k),"DisplayName",sprintf("Sim_%d",k));
                    end
